...
def get_sequence_actual_length(tensor, dim=1):
    """
    获取tensor的真实长度
    Args:
    ...
    """
    print('length feature shape', tensor.shape)
    actual_length = tf.reduce_sum(tf.sign(tensor), axis=dim)
...
def map_domain2id(items, voc, max_len, none_word=1, lower=False, init_value=0, allow_error=True,domain_len=83):
    """
    将word/pos等映射为id
    Args:
        domain_len : domain的数目
        items: list, 待映射列表
        voc: 词表
        max_len: int, 序列最大长度
        none_word: 未登录词标号,默认为0
    ...
    """
    assert type(none_word) == int
    arr = np.zeros((domain_len,max_len), dtype='int32') + init_value
    min_range = min(max_len, len(items))
    for j in range(min_range):  # 若items长度大于max_len，则被截断
        item = items[j] if not lower else items[j].lower()
        list_from_item = item.split(',')
        for domain in list_from_item:
            i = voc[domain]
            arr[i][j] = 1
    # return np.concatenate(arr) #47*83
    return arr
...
def label2onehot(labels,batch_size,seq_length,n_class):
    # labels = np.zeros((2,3),dtype=np.int)
    # labels[1][2] = 3
    # labels[1][0] = 6
    # labels[0][2] = 5
    # print(labels.shape)
    ...
    one_hot_label = np.zeros((batch_size,seq_length,n_class),dtype=np.int)

    # one_hot_label[np.arange(0,2),np.arange(0,3), labels] = 1
    # one_hot_label[1,1,1] =1
    for i in range(0,batch_size):
        for j in range(0,seq_length):
            label = labels[i][j]
            print(label)
            one_hot_label[i][j][label] = 1
    # for i in range(0,2):
    #     one_hot_label[i ,np.arange(0, 3), labels[i]] = 1
    # print(one_hot_label)
