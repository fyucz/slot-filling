...
class SequenceLabelingModel2(object):
    ...
    def build_model(self):
        ...
        # cross_entropy = -tf.reduce_sum(labels * tf.log(slot_logits), axis=2)
        ...
        softmax_loss = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=slot_logits)
        self.slot_loss = tf.reduce_sum(softmax_loss * mask) / tf.reduce_sum(mask)
        # self.slot_loss = tf.reduce_mean(cross_entropy_masked)
        self.total_loss = self.slot_loss
        self.train_loss = self.slot_loss
        ...
        for step in range(self._nb_epoch):
            ...
            with tqdm(total=train_iter.n_samples) as pbar:
                ...
                for batch_iter in train_iter.next():
                    ...
                    _, loss, intent_loss, acc, batch_count, intent_preds, slot_logits, seq_lengths \
                        = self.sess.run([self.train_op,
                                        ...
                                        feed_dict=feed_dict)
                    pred_slots = self.bilstm_infer(slot_logits, seq_lengths)
                    # print(slot_logits)
    ...
    def evaluate(self, dev_iter):
        ...
        for batch_iter in dev_iter.next():
            ...
            loss, intent_loss, acc, batch_count, slot_logits, seq_lengths = \
    ...
    def crf_infer(self, slot_logits, slot_lengths, trans_params):
        ...
        return pred_slots
        pass
    def bilstm_infer(self,slot_logits, slot_lengths):
        batch_size = slot_lengths.shape[0]
        # print('slot_logits shape',slot_logits.shape)
        # print('slot_lengths shape', slot_lengths.shape)
        pred_slots = []
        for i in range(batch_size):
            ...
            session_slots = np.argmax(logit_actual, axis=1).tolist()
    ...
    def predict(self, data_test_dict):
        ...
        with tqdm(total=infer_iter.n_samples) as pbar:
            for batch_iter in infer_iter.next():
                ...
                pred_intents, batch_count, slot_logits, seq_lengths = \
